...
export function parseSequenceSource(
  ...
  const ensureParticipant = (
    ...
    if (participantIndex.has(rawId) || participantIndex.has(id)) {
      // Prefer original raw key tracking
      const existing =
        participants[participantIndex.get(rawId) ?? participantIndex.get(id)!]
      if (label && existing && existing.label === existing.id) {
    ...
    if (participants.some((p) => p.id === id)) {
      diagnostics.push({
        severity: 'error',
        code: 'id-collision',
        message: `Participant id collision after sanitization: "${rawId}" → "${id}"`,
        range: range(lineNo),
      })
      return id
    }
    ...
    participantIndex.set(id, participants.length)
    participants.push(p)
    return id
  }
