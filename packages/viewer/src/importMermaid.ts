...
function sanitizeId(raw: string): string {
  return raw.replace(/[^a-zA-Z0-9_-]/g, '_') || 'node'
}

function createIdRegistry(warnings: string[], reserved: string[] = []) {
  const allocated = new Map<string, string>()
  const used = new Set(reserved)

  return (raw: string): string => {
    const existing = allocated.get(raw)
    ...
    if (id !== base) {
      warnings.push(`Mermaid identifier "${raw}" was mapped to "${id}" to avoid an Arc ID collision`)
    }

    allocated.set(raw, id)
    used.add(id)
    return id
  }
}
...
function parseFlowchart(
  ...
  const idFor = createIdRegistry(warnings)
  const size = options.defaultSize ?? 'm'
  const declaredDirection = lines[0].match(/^(?:flowchart|graph)\s+(TD|TB|BT|LR|RL)\b/i)?.[1]?.toUpperCase()
  if (declaredDirection && declaredDirection !== 'LR') {
  ...
  for (const id of nodeLabels.keys()) allIds.add(id)
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (pMatch) {
      ...
      if (!participantIds.has(id)) {
        participants.push({ id: idFor(id), label })
    ...
    if (msgMatch) {
      const fromId = idFor(msgMatch[1])
      const arrow = msgMatch[2]
      const toId = idFor(msgMatch[3])
...
function parseStateDiagram(
  ...
  const idFor = createIdRegistry(warnings, ['__start__', '__end__'])
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (stateMatch) {
      stateLabels.set(idFor(stateMatch[2]), stateMatch[1])
      continue
    }
    ...
    if (transMatch) {
      ...
      transitions.push({
        from: fromRaw === '[*]' ? '__start__' : idFor(fromRaw),
        to: toRaw === '[*]' ? '__end__' : idFor(toRaw),
        label,
      })

      // Register states
      if (fromRaw !== '[*]') {
        const fromId = idFor(fromRaw)
        if (!stateLabels.has(fromId)) stateLabels.set(fromId, fromRaw)
      }
      if (toRaw !== '[*]') {
        const toId = idFor(toRaw)
        if (!stateLabels.has(toId)) stateLabels.set(toId, toRaw)
      }
      continue
    }
  ...
  for (const id of stateLabels.keys()) allIds.add(id)
